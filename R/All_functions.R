...
Generating_Zi=function(net,pirhopair,wholeindex)
{
  z=wholeindex
  znew=c()
...
HODCMclust=function(mclust,rstat)
{
  ...
  if (length(mclust$parameter$mean)==2) {
    ...
    repeat{
      ...
      if (length(lmin)!=0){lmin=sample(lmin,1)}
...
#####Iteration3_Mclust
Iteration3_Mclust<-function(iter,wholeindex,hodcmclust,net,pirhopair,choice,rstat,show.steps,trace){
  z<-wholeindex
...
#####DPdensity
#####DPdensity
DPdensitycluster<-function(v,rstat){
  results<-list()
